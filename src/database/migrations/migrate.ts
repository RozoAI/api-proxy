...
export class MigrationRunner {
  ...
  async runMigrations(): Promise<void> {
    ...
    for (const migration of pendingMigrations) {
      try {
        ...
        await this.connection.execute('INSERT INTO migrations (version, filename) VALUES (?, ?)', [
          migration.version,
          migration.filename,
        ]);
  ...
  async rollbackLastMigration(): Promise<void> {
    ...
    try {
      ...
      if (lastMigration === '001_create_payments_table') {
        ...
        try {
          await this.connection.execute(
            'ALTER TABLE payments DROP INDEX idx_payments_status_updated_at'
          );
        } catch (e) {
          /* Index might not exist */
        }
        try {
          await this.connection.execute(
            'ALTER TABLE payments DROP INDEX idx_payments_status_updated'
          );
        } catch (e) {
          /* Index might not exist */
        }
      ...
      await this.connection.execute('DELETE FROM migrations WHERE version = ?', [lastMigration]);
