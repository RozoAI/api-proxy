...
import { PaymentsRepository, PaymentRecord } from '../database/repositories/payments-repository';
...
export class PaymentService {
      ...
      if (cachedPayment) {
        ...
        if (this.paymentsRepository.isPaymentStale(cachedPayment, 15)) {
          ...
          try {
            ...
            console.warn(`[PaymentService] Failed to fetch fresh data for ${paymentId}, returning cached data:`, providerError);
        ...
        console.log(`[PaymentService] Payment ${paymentId} not found in database, fetching from provider`);
  ...
  async getPaymentByExternalId(externalId: string, chainId?: number): Promise<PaymentResponse> {
    try {
      ...
      if (cachedPayment) {
        ...
        // Check if payment is stale (status is 'started' and > 15 mins old)
        if (this.paymentsRepository.isPaymentStale(cachedPayment, 15)) {
          console.log(`[PaymentService] Payment with external ID ${externalId} is stale, fetching fresh data`);
          ...
          try {
            ...
            console.warn(`[PaymentService] Failed to fetch fresh data for external ID ${externalId}, returning cached data:`, providerError);
            // Return cached data if provider fails
            return this.paymentsRepository.convertToPaymentResponse(cachedPayment);
          }
        } else {
        ...
        // Payment not in database, fetch from provider
        console.log(`[PaymentService] Payment with external ID ${externalId} not found in database, fetching from provider`);
  ...
  async updatePaymentStatus(
    ...
    try {
      ...
      if (updated) {
        ...
        console.warn(`[PaymentService] Failed to update payment ${paymentId} - not found in database`);
      }
  ...
  /**
   * Get payments by status (for internal use)
   */
  async getPaymentsByStatus(status: string, olderThanMinutes?: number) {
