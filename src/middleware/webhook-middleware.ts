...
export function webhookLogger(
  ...
  console.log(
    `[WebhookMiddleware] ${webhookId} - Incoming ${req.method} ${req.path} from ${req.ip}`
  );
  ...
  res.on('finish', () => {
    ...
    if (res.statusCode >= 400) {
      console.error(
        `[WebhookMiddleware] ${webhookId} - Error response - Full payload:`,
...
export function webhookErrorHandler(
  ...
  console.error(
    `[WebhookMiddleware] ${webhookId} - Stack trace:`,
...
export function webhookMonitoring(
  ...
  res.on('finish', () => {
    ...
    console.log(
      `[WebhookMetrics] ${webhookId} - Provider: ${req.path.split('/')[2]}, Success: ${success}, Duration: ${duration}ms, Status: ${res.statusCode}`
    );
