...
export class MugglePayProvider extends BaseProvider {
  ...
  private transformToMugglePayRequest(paymentData: PaymentRequest): any {
    ...
    return {
      ...
      price_amount: parseFloat(paymentData.display.paymentValue || paymentData.destination.amountUnits || '0'),
      ...
      description: `${paymentData.display.intent} - ${paymentData.display.paymentValue || paymentData.destination.amountUnits} ${paymentData.display.currency}`,
  ...
  private transformFromMugglePayResponse(
    ...
    return {
      ...
      destination: {
        ...
        amountUnits: originalRequest?.display?.paymentValue || order.price_amount?.toString() || originalRequest?.destination.amountUnits || '', // 支付金额
        tokenSymbol: originalRequest?.destination?.tokenSymbol || this.mapMugglePayTokenToSymbol(invoice.pay_currency || paymentToken), // 提现链token
        tokenAddress: originalRequest?.destination?.tokenAddress || this.getTokenAddress(invoice.pay_currency || paymentToken), // 提现链token地址
      },
