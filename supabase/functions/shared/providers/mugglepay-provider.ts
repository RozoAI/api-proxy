...
export class MugglePayProvider extends BaseProvider {
  ...
  private transformFromMugglePayResponse(
    ...
    return {
      ...
      createdAt: mugglePayResponse.created_at
        ? new Date(mugglePayResponse.created_at).toISOString()
      ...
      destination: {
        ...
        chainId: originalRequest?.destination?.chainId || originalRequest?.preferredChain || this.getChainIdFromToken(mugglePayResponse.pay_currency || mugglePayResponse.token),
        amountUnits: originalRequest?.destination.amountUnits || mugglePayResponse.price_amount?.toString() || '',
        tokenSymbol: originalRequest?.destination?.tokenSymbol || this.mapMugglePayTokenToSymbol(mugglePayResponse.pay_currency || mugglePayResponse.token),
        tokenAddress: originalRequest?.destination?.tokenAddress || this.getTokenAddress(mugglePayResponse.pay_currency || mugglePayResponse.token),
      },
      externalId: originalRequest?.externalId || mugglePayResponse.metadata?.rozo_external_id || mugglePayResponse.merchant_order_id || null,
      metadata: {
        ...originalRequest?.metadata,
        ...mugglePayResponse.metadata?.original_metadata,
        webhookUrl: this.getCallbackUrl(),
        provider: 'mugglepay',
        receivingAddress: mugglePayResponse.address || mugglePayResponse.receiving_address || null,
        memo: mugglePayResponse.memo || null,
        payinchainid: originalRequest?.preferredChain || this.getChainIdFromToken(mugglePayResponse.pay_currency || mugglePayResponse.token),
        payintokenaddress: this.getTokenAddress(paymentToken),
        mugglepay_order_id: mugglePayResponse.order_id || mugglePayResponse.id,
        mugglepay_status: mugglePayResponse.status,
        mugglepay_pay_currency: mugglePayResponse.pay_currency || mugglePayResponse.token || paymentToken,
        mugglepay_callback_url: mugglePayResponse.callback_url,
        mugglepay_merchant_order_id: mugglePayResponse.merchant_order_id,
      },
  ...
  private getCallbackUrl(): string {
    ...
    // const baseUrl = 'https://9801fe63f3f5.ngrok-free.app';
